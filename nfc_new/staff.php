...
<body>
   ...
   <div class="topbar">
      ...
      <div class="topbar-right">
         ...
         <a href="index.php" class="logout-btn" onclick="doLogout()"><i class="fas fa-sign-out-alt"></i>Logout</a>
      </div>
   </div>
   ...
   <script>
      ...
      if(!role || (role!=='staff' && role!=='admin')){window.location.href='login.php';}
      document.getElementById('staffName').textContent = user || 'Staff';
      ...
      if(orders.length===0){
         orders = [
            {id:'ORD-001',customer:'Ahmad Razif',items:'2x Classic Chicken Burger, 1x Coca-Cola',total:'RM21.70',time:'10:32 AM',status:'pending'},
            {id:'ORD-002',customer:'Siti Aisyah',items:'1x 2-pc Fried Chicken, 1x Iced Lemon Tea',total:'RM17.80',time:'10:45 AM',status:'preparing'},
            {id:'ORD-003',customer:'Lim Wei Jian',items:'3x Popcorn Chicken, 2x Sprite',total:'RM34.50',time:'11:00 AM',status:'ready'},
         ];
      }
      ...
      function actionBtns(idx,s){
         if(s==='pending') return '<button class="action-btn btn-prepare" onclick="updateStatus('+idx+',\'preparing\')">Start Prep</button>';
         if(s==='preparing') return '<button class="action-btn btn-ready" onclick="updateStatus('+idx+',\'ready\')">Mark Ready</button>';
         if(s==='ready') return '<button class="action-btn btn-done" onclick="updateStatus('+idx+',\'completed\')">Done</button>';
         return '<span style="color:#bbb;font-size:.78rem;">—</span>';
      }

      function updateStatus(idx,status){
         orders[idx].status=status;
         localStorage.setItem('nfc_orders',JSON.stringify(orders));
         loadOrders();
      }
      ...
      function loadOrders(){
         ...
         if(active.length===0){
            ...
            tbody.innerHTML=active.map(function(o,i){
               var realIdx=orders.indexOf(o);
               return '<tr><td><strong>'+o.id+'</strong></td><td>'+o.customer+'</td><td style="max-width:220px;white-space:normal;">'+o.items+'</td><td><strong>'+o.total+'</strong></td><td>'+o.time+'</td><td>'+statusBadge(o.status)+'</td><td>'+actionBtns(realIdx,o.status)+'</td></tr>';
            }).join('');
         }
         var cnt={pending:0,preparing:0,ready:0};
         orders.forEach(function(o){if(cnt[o.status]!==undefined)cnt[o.status]++;});
